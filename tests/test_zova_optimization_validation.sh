...
write_report pure "$CBM_ZOVA_OPTIMIZATION_PURE_REPORT"
write_report single "$CBM_ZOVA_OPTIMIZATION_SINGLE_REPORT"
SH
...
[[ $(wc -l <"$TMP/record.states" | tr -d ' ') == 2 ]] || fail "wrong workload count"
state1=$(sed -n '1p' "$TMP/record.states")
state2=$(sed -n '2p' "$TMP/record.states")
[[ $state1 == full\|CBM_MODE_FULL\|7\|source-change\|* ]] || fail "full pair not first"
[[ $state2 == incremental\|CBM_MODE_INCREMENTAL\|99\|source-change\|* ]] ||
  fail "pure incremental did not reuse mutated repository"
[[ ${state2##*|} == "${state1##*|}" ]] || fail "paired cache root was not reused"
for report in pure-full single-full pure-incremental single-incremental; do
  [[ -s "$TMP/success/$report.json" ]] || fail "$report report missing"
done
